...
def test_train_hardlim_delta():
    ...
    for i in range(10):
        ...
        out = model.predict(X_train)
        model.set_weights(np.random.randn(*model.get_weights().shape))
        model.train(X_train, out, batch_size=10, num_epochs=300, alpha=0.1, gamma=0.1, learning="Delta")
        new_out = model.predict(X_train)
        np.testing.assert_array_almost_equal(out, new_out, decimal=4)


def test_calculate_mean_squared_error() -> object:
    input_dimensions = 5
    number_of_nodes = 5
...
def main():
    test_weights()
    test_predict_linear()
    test_predict_hard_limit()
    test_pseudo_inverse_fit()
    test_calculate_mean_squared_error()
    test_train_linear_delta()
    test_train_hardlim_delta()
